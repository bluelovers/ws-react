...
const RandomProvider = ({ children, initState, key, seed, }) => {
    const [randomState, setRandomState] = (0, react_1.useState)(initState || true);
    const defaults = (0, react_1.useMemo)(() => {
        seed || (seed = (0, nanoid_1.nanoid)());
        key || (key = (0, nanoid_1.nanoid)());
        let initState = randomState;
        if (initState === true) {
            initState = (0, seedrandom_1.default)(seed, {
                state: initState,
            }).state();
        }
        else {
